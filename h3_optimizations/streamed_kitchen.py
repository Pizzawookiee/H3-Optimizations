...
This test-branch port uses a native Q-only entry point and exact two-pass V staging.
...
# Deliberately no stage-aware weight prefetch in the test-branch port.
def begin_stage_prefetch(*_args, **_kwargs):
    return None

def wait_stage_prefetch(*_args, **_kwargs):
    return None

def release_stage_prefetch(*_args, **_kwargs):
    return None

def abandon_stage_prefetch(*_args, **_kwargs):
    return None

def stage_prefetch_enabled(*_args, **_kwargs):
    return False
...
CARRIER_Q_TILE = 128
QUERY_CHUNK_ALIGNMENT = CARRIER_Q_TILE
...
def _query_stub(q, rows=CARRIER_Q_TILE):
    if q.shape[-2] >= rows:
        return q[..., :rows, :]
...
def run_streamed_kitchen_qkv(
    ...
    *,
    ...
    query_chunk_rows,
    strided_qk_input,
    routing_q_tile=PRODUCTION_Q_TILE,
    routing_kv_tile=PRODUCTION_KV_TILE,
    ...
    stub_shape = (1, heads, CARRIER_Q_TILE, head_dim)
    try:
...
def _route_chunk(route, start, end):
    ...
    if start % route_q_tile:
        raise ValueError(
            'streamed query start must align to route Q tile %d rows'
            % route_q_tile
        )
    ...
    *,
    ...
    try:
        ...
        return run_streamed_kitchen_qkv(
            ...
            routing_q_tile=(
                self.q_tile if self.q_tile is not None else PRODUCTION_Q_TILE
            ),
            routing_kv_tile=(
                self.kv_tile if self.kv_tile is not None else PRODUCTION_KV_TILE
            ),
            stage_prefetch=False,
