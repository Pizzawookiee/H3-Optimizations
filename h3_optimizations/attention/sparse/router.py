...
class SparseTileRouter:
    ...
    def build_lut_rows_from_summaries(
        self,
        q_summary,
        k_summary,
        layout,
        ...
        *,
        ...
        """Build only a contiguous global range of Q-tile route rows.

        Top-K selection is independent for every Q tile, so a bounded Q-summary
        slab can be routed immediately against the complete K-summary tensor.
        KV indices remain global. Different matmul M may change last-bit
        reductions, so this is selection-equivalent but not promised bit-exact.
        """
        if q_summary.ndim != 4 or k_summary.ndim != 4:
            raise SparseRouterError(
                'tile router summaries must be rank-4 HND tensors'
        ...
        if (
            q_tile_start < 0
            or local_q_tiles <= 0
            or q_tile_stop > geometry.q_tiles
        ):
            raise SparseRouterError(
        ...
        if sparse_global_start < q_tile_stop:
            ...
            sparse_rows = self._pack_rows(
                indices, geometry, dense, dense_delta
            )
            lut[
                ...,
                local_sparse_start:,
