'''Chunked INT8 Q/K/V production for the H3 Triton sparse fallback.'''
...
def _empty_carrier(*, device, dtype, sequence, heads, layer_index):
    ...
    return PreparedTritonSparseQKV(
        ...
        q_scale=torch.empty(
            (1, heads, q_blocks), dtype=torch.float32, device=device
        ),
        k_int8=torch.empty(shape, dtype=torch.int8, device=device),
        k_scale=torch.empty(
            (1, heads, kv_blocks), dtype=torch.float32, device=device
        ),
...
if TRITON_AVAILABLE:
    ...
    def _pack_v_int8_chunk_kernel(
        ...
        tl.store(scale_destination, scale)
...
def pack_triton_v_chunk_into(
    ...
    *,
    ...
    if not x.is_cuda or x.ndim != 4 or x.stride(-1) != 1:
        raise TritonSparseQKVError(
            'Triton sparse V input must be a CUDA HND tensor'
        )
    ...
    if (
        tuple(scales.shape) != (batch, heads, total_blocks, head_dim)
    ...
    _pack_v_int8_chunk_kernel[(blocks, heads, batch)](
        x,
        output,
        scales,
        x.stride(0),
...
def _pack_reference_blocks(x, block_size):
    ...
    summaries = torch.empty(
        (batch, heads, blocks, head_dim), dtype=x.dtype
    )
...
def _pack_reference_v(x, block_size):
    batch, heads, sequence, head_dim = x.shape
    blocks = (sequence + block_size - 1) // block_size
    output = torch.empty_like(x, dtype=torch.int8)
    scales = torch.empty((batch, heads, blocks, head_dim), dtype=torch.float32)
    for block in range(blocks):
        start = block * block_size
        end = min(start + block_size, sequence)
        value = x[..., start:end, :].float()
        scale = value.abs().amax(dim=-2) / 127.0 + 1e-7
        quantized = value / scale[..., None, :]
        quantized += 0.5 * torch.where(quantized >= 0, 1.0, -1.0)
        output[..., start:end, :].copy_(quantized.to(torch.int8))
        scales[..., block, :].copy_(scale)
...
def pack_float_qkv(q, k, v, *, layer_index):
    ...
    if not q.is_cuda:
        ...
        v_i8, v_scale = _pack_reference_v(v, KV_TILE)
...
def run_chunked_triton_sparse_qkv(
    ...
    *,
    ...
    if sequence <= 0 or int(module.head_dim) != HEAD_DIM:
        raise TritonSparseQKVError(
            'chunked Triton sparse QKV requires H3 head_dim 128'
        )
