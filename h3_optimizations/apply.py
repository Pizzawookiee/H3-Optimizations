...
from .attention.sparse.fused_qkv import TRITON_AVAILABLE as SPARSE_TRITON_AVAILABLE
from .dense_resolver import install_dense_attention, preserve_dense_attention, resolve_dense_attention
...
from .plan import DENSITY_FIXED, FUSED_QKV_AUTO, FUSED_QKV_OFF, H3OptimizationPlan, PLAN_KEY, STATUS_KEY
...
from .runtime.context import H3RuntimeSession, RUNTIME_SESSION_KEY, install_runtime_wrapper
from .v_layout_compat import install_v_layout_compat, not_applicable_v_layout
...
def _fp8_execution_available(environment):
    ...
    return bool(getattr(environment, 'cuda_available', False)) and capability is not None and tuple(capability) >= (8, 9)


def _resolve_dense(plan, model, inventory, environment=None):
    memory = plan.memory
    dense = preserve_dense_attention('no memory optimization requested') if memory is None else resolve_dense_attention(model)
    qkv = resolve_qkv_provider(
        inventory,
        request=_qkv_request(plan),
        backend_kind=dense.backend_kind,
        kitchen_producer_available=producer_api_available(device=getattr(environment, 'device_index', None)),
        memory_optimize=memory is not None,
        fp8_available=_fp8_execution_available(environment),
    )
    backend = None
    projector = None
    if qkv.provider_id in (QKV_DENSE_KITCHEN_CHUNKED, QKV_DENSE_FP8_CHUNKED):
        backend = ChunkedKitchenAttentionBackend()
        projector = ChunkedKitchenQKVProjector(fp8_projection=qkv.provider_id == QKV_DENSE_FP8_CHUNKED)
...
def _resolve_fp8_flex(plan, environment, inventory, fallback_reason, dense_attention):
    spec = preflight_fp8_flex(
        cuda_available=lambda: environment.cuda_available,
        capability_getter=lambda: environment.capability,
        device=getattr(environment, 'device_index', None),
    )
    qkv = resolve_qkv_provider(inventory, request=_qkv_request(plan), backend_kind=ATTENTION_FP8_FLEX)
    config = HybridSparseConfig(
        mode=MODE_SAGE128,
        video_budget=float(plan.sparse.video_budget),
        density_mode=DENSITY_FIXED,
...
def _resolve_triton_sparse(plan, environment, inventory, sparse_error):
    ...
    backend = TritonSparseBackend(config, spec=spec, projector=projector)
    return ResolvedAttention(
        requested=ATTENTION_SPARSE,
        selected=ATTENTION_TRITON_SPARSE,
        backend=backend,
        reason='Sparse Sage unavailable: %s; using INT8 Triton sparse attention' % sparse_error,
        backend_kind=ATTENTION_TRITON_SPARSE,
        projector=projector,
    ), qkv
...
def _resolve_attention(plan, model, inventory, environment):
    dense_attention, dense_qkv = _resolve_dense(plan, model, inventory, environment)
    ...
    try:
        ...
        try:
            return _resolve_triton_sparse(plan, environment, inventory, sparse_exc)
        except TritonSparseError as triton_exc:
            fallback_reason = 'Sparse Sage unavailable: %s; INT8 Triton unavailable: %s' % (sparse_exc, triton_exc)
            try:
                return _resolve_fp8_flex(plan, environment, inventory, fallback_reason, dense_attention)
            except FP8FlexError as flex_exc:
                return ResolvedAttention(
                    requested=ATTENTION_SPARSE,
                    selected=dense_attention.selected,
                    backend=dense_attention.backend,
                    reason='%s; FP8 FlexAttention unavailable: %s; %s' % (fallback_reason, flex_exc, dense_attention.reason),
...
def _ensure_sparse_runtime(model_patcher):
    options = model_patcher.model_options['transformer_options'] = model_patcher.model_options.get('transformer_options', {}).copy()
    session = options.get(RUNTIME_SESSION_KEY)
    if session is not None:
        if not isinstance(session, H3RuntimeSession):
            raise TypeError('%s is not an H3 Optimizations runtime session' % RUNTIME_SESSION_KEY)
...
def _inventory_status(inventory):
    return {'qkv': list(inventory.labels('qkv')), 'fc1': list(inventory.labels('fc1')), 'fc2': list(inventory.labels('fc2'))}


def _status(plan, environment, attention, qkv, mlp, *, attention_blocks, mlp_blocks, runtime_installed, inventory, v_layout):
    return {
        'plan_version': int(plan.version),
        'plan_signature': plan.signature,
        'attention': {'requested': attention.requested, 'selected': attention.selected, 'reason': attention.reason, 'patched_blocks': int(attention_blocks)},
        'v_layout': {'state': v_layout.state, 'reason': v_layout.reason, 'patched_blocks': int(v_layout.patched_blocks)},
        'sparse': None if plan.sparse is None else {'video_budget': float(plan.sparse.video_budget), 'denser_early_late_steps': bool(plan.sparse.denser_early_late_steps)},
        'fused_qkv': {
            ...
            'producer_abi': KITCHEN_PRODUCER_ABI if qkv.provider_id in (QKV_DENSE_KITCHEN_CHUNKED, QKV_DENSE_FP8_CHUNKED) else None,
        },
        'mlp': {
            ...
            'reason': mlp.reason,
            'chunk_rows': None if plan.memory is None else int(plan.memory.chunk_rows),
        ...
        'device': {
            ...
            'capability': None if environment.capability is None else [int(value) for value in environment.capability],
        },
    }
...
def apply_plan(model, plan: H3OptimizationPlan):
    ...
    patched = model.clone()
    attention_blocks = 0
    sparse_execution_selected = attention.backend_kind in (ATTENTION_SPARSE, ATTENTION_TRITON_SPARSE, ATTENTION_FP8_FLEX)
    if sparse_execution_selected:
        if attention.backend_kind == ATTENTION_FP8_FLEX:
            _backend, attention_blocks = configure_backend(
                patched, attention.backend, projector=attention.projector, backend_fallback_to_dense=True
            )
        else:
            _backend, attention_blocks = configure_backend(patched, attention.backend, projector=attention.projector)
        if attention.backend_kind == ATTENTION_FP8_FLEX and attention.dense_resolution is not None:
            install_dense_attention(patched, attention.dense_resolution)
        v_layout = not_applicable_v_layout('%s owns the main H3 forward' % attention.selected)
        ...
        if qkv.provider_id in (QKV_DENSE_KITCHEN_CHUNKED, QKV_DENSE_FP8_CHUNKED):
            ...
            v_layout = not_applicable_v_layout('chunked Kitchen QKV owns the main H3 forward')
        else:
            v_layout = install_v_layout_compat(patched)
            attention_blocks = int(v_layout.patched_blocks)
        install_dense_attention(patched, attention.dense_resolution)
    else:
        v_layout = not_applicable_v_layout('no H3 Memory Optimization request')

    mlp, mlp_blocks = _install_mlp(patched, plan, inventory, environment)
    ...
    options = patched.model_options['transformer_options'] = patched.model_options.get('transformer_options', {}).copy()
    ...
    logging.info('%s armed: attention=%s v_layout=%s qkv=%s mlp=%s device=%s', LOG_PREFIX, attention.selected, v_layout.state, qkv.provider_id, mlp.provider_id, environment.device_name)
    return patched
