...
// Benchmark-only side-car entry point for proving that independently streamed
// Q chunks reproduce the coupled Q/K quantizer byte for byte.  Production does
// not call this launcher until the complete streamed schedule earns its keep.
void launch_quant_q_per_thread_int8(
  ...
  // The coupled C=128 quantizer selects its Q rotation from the full K length.
  DISPATCH_FP_DTYPE(input_dtype_code, T, [&] {
    if (full_Lk <= 256) {
      quant_q_kernel<T, 4, BLKQ, WARPQ, CHANNEL_TILES, 4, true>
          <<<grid, 128, 0, stream>>>(
              static_cast<const T *>(q), static_cast<int8_t *>(q_int8),
              static_cast<float *>(q_scale), Lq, C, H_q, q_sc_per_h,
              q_stride_b, q_stride_h, q_stride_n);
    } else {
